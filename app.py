...
MODAL_SYSTEM_JS = """
<div id="custom-modal" class="fixed inset-0 bg-black/80 backdrop-blur-sm z-[100] hidden items-center justify-center opacity-0 transition-opacity duration-300">
        ...
        function renderMenu(category) {
            ...
            grid.innerHTML = availableItems.map(item => `
                <div class="bg-gray-900 border border-gray-800 rounded-2xl overflow-hidden flex flex-col shadow-lg hover:border-gray-700 transition">
                    ${item.image ? `<img src="${item.image}" class="w-full h-44 object-cover">` : `<div class="w-full h-44 bg-gray-800 flex items-center justify-center text-gray-600"><i class="fa-solid fa-utensils text-4xl"></i></div>`}
...
<body class="bg-gray-950 text-gray-100 font-sans min-h-screen flex flex-col">
    ...
    <div class="bg-gray-900/50 border-b border-gray-800 px-6 flex overflow-x-auto gap-4">
        ...
        <button onclick="switchTab('system-tab')" class="tab-btn px-4 py-3.5 font-bold text-sm border-b-2 border-transparent text-gray-400 hover:text-white transition">Система / Резервні копії</button>
    </div>

    <main class="flex-1 p-6 max-w-7xl w-full mx-auto">
        <div id="orders-tab" class="tab-content space-y-6">
            <div class="flex justify-between items-center">
                <h2 class="text-xl font-extrabold text-white border-l-4 border-amber-500 pl-2">Поточні замовлення в реальному часі</h2>
            ...
            <div id="orders-list" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                <p class="text-gray-500 text-center py-12 col-span-full">Немає нових замовлень.</p>
            </div>
        </div>
    ...
    <script>
        ...
        socket.on('menu_sync', (menu) => {
            ...
            container.innerHTML = menu.map(i => `
                <tr class="hover:bg-gray-900/40 transition">
                    <td class="p-4">${i.image ? `<img src="${i.image}" class="w-12 h-12 object-cover rounded-lg border border-gray-700">` : `<div class="w-12 h-12 bg-gray-800 rounded-lg flex items-center justify-center text-gray-600"><i class="fa-solid fa-utensils"></i></div>`}</td>
        ...
        function convertImageToBase64(input) {
            ...
            reader.onloadend = function() {
                document.getElementById('item-image-base64').value = reader.result;
            }
        ...
        async function clearDatabase() {
            if(await customConfirm('УВАГА! Ви дійсно хочете повністю видалити всі страви, замовлення та відгуки з бази даних? Цю дію неможливо скасувати!', 'Очищення бази', true)) {
                ...
                // Додаємо додатковий захист за допомогою prompt
        ...
        async function importDatabase() {
            ...
            if(!file) { 
                await customAlert('Будь ласка, спочатку оберіть файл резервної копії формату .json.', 'Файл не обрано'); 
                return; 
            }
